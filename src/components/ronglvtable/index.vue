        ...
        <table cellpadding="0px" cellspacing="0px" class="table-list">
            <thead class="table-header">
                <tr class="table-header-contant">
                    <th class="header-item" v-for="(index, item) in tableData[0]" :key="index">
                        <div class="item">
                            <div>{{ item.name }} </div>
                            <div v-if="item.sort" class="arrow" @click="clickSort(item.label)">
                                <img class="up" :src="sort === `up_${item.label}` ? arrow_up_active : arrow_up" alt="">
                                <img class="down" :src="sort === `down_${item.label}` ? arrow_down_active : arrow_down" alt="">
                            </div>
                        </div>
                    </th>
                    <th class="header-item" style="text-align:center; min-width: 8em">操作</th>
                </tr>
            </thead>
    ...
    methods: {
        ...
        sortData(order, label) {
            this.tableData && this.tableData[1] && this.tableData[1].sort((a, b) =>
                order === 'up' ? a[label] - b[label] : b[label] - a[label]
            )
        },
        clickSort(label) {
            let patt = /^up_/
